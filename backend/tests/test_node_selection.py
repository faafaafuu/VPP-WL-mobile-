...
def node(tag: str, priority: int, health_score: int, status: NodeStatus = NodeStatus.ACTIVE) -> VpnNode:
    return VpnNode(
        id=tag,
        tag=tag,
        region="eu",
...
class NodeSelectionTest(unittest.TestCase):
    def test_prefers_priority_then_health_and_ignores_unusable_nodes(self) -> None:
        ...
        self.assertEqual([item.tag for item in selected], ["p1-good", "p2"])
